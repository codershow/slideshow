window.addEventListener('load',function () {
    ...
    for (var i = 0; i < ul.children.length; i++) {
        ...
        // 把小li添加到  ol 里面
        ol.appendChild(li);
        // 4. 小圈圈的排他思想 可以在所有小li生成的同时绑定点击事件
    li.addEventListener('click',function () {
        // 干掉所有人 把所有小li清除 current类名
    ...
    arrow_r.addEventListener('click',function () {
        if (flag) {
            flag = false;  // 关闭节流阀 
            // 如果走到最后一张复制的图片，此时 我们的 ul 要快速复原 left 改为 0
        ...
        // 8. 点击右侧按钮，小圆圈跟随一起变化 可以再声明一个变量控制小圆圈的播放 但必须是全局变量
        circle++;
    ...
    arrow_1.addEventListener('click',function () {
        if (flag) {
            ...
            // 8. 点击左侧按钮，小圆圈跟随一起变化 可以再声明一个变量控制小圆圈的播放 但必须是全局变量
            circle--;
            // 如果circle < 0 说明第一张图片，则小圆圈要改为第四个小圈圈
            if (circle < 0) {
